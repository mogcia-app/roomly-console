...
async function sendEmailToHotel({ body, guestName, hotelId, hotelLabel, roomLabel, threadId, title }) {
  ...
  const introText = emergencyLabel
    ? "緊急対応が必要なメッセージを受信しました。内容を確認のうえ、至急ご対応ください。"
    : `${hotelLabel} のチャットに新しいメッセージが届きました。`;
  ...
  const text = [
    ...
    emergencyLabel ? `緊急カテゴリ: ${emergencyLabel}` : "",
    `内容: ${body}`,
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; line-height: 1.6; color: #1c1917;">
      ...
      ${emergencyLabel ? `<p style="margin: 0 0 8px;"><strong>緊急カテゴリ:</strong> ${safeEmergencyLabel}</p>` : ""}
